...
fun ggml_vec_add_f16(n: Int, z: ShortArray, x: ShortArray, y: ShortArray) {
    for (i in 0 until n) z[i] = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
        io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]) + io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(
            y[i]
        )
    )
}
...
fun ggml_vec_neg_f16(n: Int, y: ShortArray, x: ShortArray) {
    for (i in 0 until n) y[i] = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
        -io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i])
    )
}
fun ggml_vec_sqr_f16(n: Int, y: ShortArray, x: ShortArray) {
    for (i in 0 until n) { val v = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]); y[i] =
        io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(v * v)
    }
}
...
fun ggml_vec_sgn_f16(n: Int, y: ShortArray, x: ShortArray) {
    for (i in 0 until n) { val v = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]); y[i] =
        io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(if (v > 0f) 1f else if (v < 0f) -1f else 0f)
    }
}
fun ggml_vec_step_f16(n: Int, y: ShortArray, x: ShortArray) {
    for (i in 0 until n) y[i] = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
        if (io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]) > 0f) 1f else 0f
    )
}
...
fun ggml_vec_mad_f16(n: Int, y: ShortArray, x: ShortArray, v: Float) {
    for (i in 0 until n) y[i] = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
        io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(y[i]) + io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(
            x[i]
        ) * v
    )
}
...
fun ggml_vec_scale_f16(n: Int, y: ShortArray, v: Float) {
    for (i in 0 until n) y[i] = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
        io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(y[i]) * v
    )
}
...
fun ggml_vec_sum_f16_ggf(n: Int, s: FloatArray, x: ShortArray) { var sum = 0.0f; for (i in 0 until n) sum += io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(
    x[i]
); s[0] = sum }
fun ggml_vec_sum_bf16_ggf(n: Int, s: FloatArray, x: ShortArray) { var sum = 0.0f; for (i in 0 until n) sum += io.github.kotlinmania.llama.core.ggml_compute_bf16_to_fp32(
...
fun ggml_vec_dot_f16(n: Int, s: FloatArray, bs: Int, x: ShortArray, bx: Int, y: ShortArray, by: Int, nrc: Int) {
    require(nrc == 1)
    var sum = 0.0f
    for (i in 0 until n) sum += io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]) * io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(
        y[i]
    )
    s[0] = sum
}
...
fun ggml_vec_dot_bf16(n: Int, s: FloatArray, bs: Int, x: ShortArray, bx: Int, y: ShortArray, by: Int, nrc: Int) {
    require(nrc == 1)
    var sum = 0.0f
    for (i in 0 until n) sum += io.github.kotlinmania.llama.core.ggml_compute_bf16_to_fp32(
        io.github.kotlinmania.llama.core.GGMLBF16(x[i].toUShort())
    ) * io.github.kotlinmania.llama.core.ggml_compute_bf16_to_fp32(
...
fun ggml_vec_dot_f16_unroll(n: Int, xs: Int, s: FloatArray, xv: Array<ShortArray>, y: ShortArray) {
    for (j in 0 until xs) {
        var sum = 0.0f
        for (i in 0 until n) sum += io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(xv[j][i]) * io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(
            y[i]
        )
        s[j] = sum
    }
...
private fun ggml_silu_f16(x: Short): Short = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
    ggml_silu_f32(
...
fun ggml_vec_relu_f16(n: Int, y: ShortArray, x: ShortArray) {
    for (i in 0 until n) { val v = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]); y[i] =
        io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(if (v > 0f) v else 0f)
    }
}
fun ggml_vec_leaky_relu_f16(n: Int, y: ShortArray, x: ShortArray, ns: Float) {
    for (i in 0 until n) { val v = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]); y[i] =
        io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(if (v > 0f) v else v * ns)
    }
}
fun ggml_vec_elu_f16(n: Int, y: ShortArray, x: ShortArray) {
    for (i in 0 until n) { val v = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]); y[i] =
        io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(if (v > 0f) v else (exp(v) - 1f))
    }
}
...
fun ggml_vec_hardsigmoid_f16(n: Int, y: ShortArray, x: ShortArray) {
    for (i in 0 until n) y[i] = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
        (io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]) / 6f + 0.5f).coerceIn(0f, 1f)
    )
}
fun ggml_vec_hardswish_f16(n: Int, y: ShortArray, x: ShortArray) {
    for (i in 0 until n) { val v = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]); y[i] =
        io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(v * (v / 6f + 0.5f).coerceIn(0f, 1f))
    }
}
...
fun ggml_vec_silu_backward_f16(n: Int, dx: ShortArray, x: ShortArray, dy: ShortArray) {
    for (i in 0 until n) {
        val xv = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i])
        val dyv = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(dy[i])
        val s = 1f / (1f + exp(-xv))
        dx[i] = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(dyv * s * (1f + xv * (1f - s)))
    }
}
fun ggml_vec_swiglu_f16(n: Int, y: ShortArray, x: ShortArray, g: ShortArray) {
    for (i in 0 until n) y[i] = io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
        ggml_silu_f32(
            io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i])
        ) * io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(g[i])
    )
}
...
fun ggml_vec_reglu_f16(n: Int, y: ShortArray, x: ShortArray, g: ShortArray) {
    for (i in 0 until n) { val v = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x[i]); y[i] =
        io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
            (if (v > 0f) v else 0f) * io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(g[i])
        )
    }
}
...
fun ggml_silu_backward_f16(x: Short, dy: Short): Short {
    val v = io.github.kotlinmania.llama.core.GGML_FP16_TO_FP32(x)
    val s = 1.0f / (1.0f + kotlin.math.exp(-v))
    return io.github.kotlinmania.llama.core.GGML_FP32_TO_FP16(
